...
def test_T002_object_put_get():
    ...
    for size, label in sizes:
        ...
        print(f"⬆️  Uploading {label} object: {key}")
        s3.put_object(Bucket=BUCKET, Key=key, Body=data)
        log.info(f"✅ Uploaded {label}")

        print(f"⬇️  Downloading {label} object: {key}")
...
def test_T003_object_list():
    print("\n▶️ T003 - Object LIST with pagination and prefix")
...
def test_T004_object_delete():
    ...
    if keys_to_delete:
        ...
        for obj in deleted:
            log.info(f"🗑️ Deleted: {obj['Key']}")
    else:
        print("ℹ️ No objects to delete.")

def test_T005_multipart_upload():
    print("\n▶️ T005 - Multipart Upload (>5GB simulated with 6MB)")
